...
class OrcColumnVisitor(val reader: PageReader, val batch: VectorizedRowBatch, val i: Integer) extends ColumnVisitor {
  override def booleanColumn(column: Column): Unit = {
    column match {
      case _ if reader.isNull(column) => {
        batch.cols(column.getIndex).noNulls = false
        batch.cols(column.getIndex).isNull.update(i, true)
      }
  ...
  override def longColumn(column: Column): Unit = {
    column match {
      ...
      case _ => batch.cols(column.getIndex).asInstanceOf[LongColumnVector].vector(i) = reader.getLong(column)
    }
  }
  ...
  override def doubleColumn(column: Column): Unit = {
    column match {
      ...
      case _ => batch.cols(column.getIndex).asInstanceOf[DoubleColumnVector].vector(i) = reader.getDouble(column)
    }
  }
  ...
  override def stringColumn(column: Column): Unit = {
    column match {
      ...
      case _ => batch.cols(column.getIndex).asInstanceOf[BytesColumnVector]
